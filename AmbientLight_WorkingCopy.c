...
#define AMBIENTPOWER_EN_TRIS			TRISBbits.TRISB5
#define AMBIENTPOWER_EN_BIT				PORTBbits.RB5

#define AMBIENTSENSE_TRIS				TRISBbits.TRISB12
#define AMBIENTSENSE_EN_BIT				AD1PCFGbits.PCFG12  
#define AMBIENTSENSE_ADC_PIN 			0x0C
...
AMBIENTPOWER_EN_TRIS		= 0;			//Output 
AMBIENTSENSE_TRIS			= 1;			//Input 


char AMBIENT_CODE_EN 	= 0;
char ReadRXBuff;
int count				= 0;
...
while(1)
 {
    ...
    if (ReadRXBuff)
    {
      ...
      {
        AD1CHSbits.CH0SA  = AMBIENTSENSE_ADC_PIN;		//Set ADC pin to next sample ambient voltage  **AMBIENT_ADC_PIN = 0x0C = 0b1100, datasheet: "Channel 0 positive input is AN12"
...
void ADC_Init (void)
{
  AMBIENTSENSE_EN_BIT			= 0;	// Pin set as analog input, A/D samples voltage
